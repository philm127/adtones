...
namespace EFMVC.Web.Areas.OperatorAdmin.Controllers
{
    ...
    [RoutePrefix("AdvertisersManagement")]
    public class AdvertisersManagementController : Controller
    {
        ...
        public AdvertisersManagementController(ICommandBus commandBus, IUserRepository userRepository, IContactsRepository contactsRepository, ICompanyDetailsRepository companydetailsRepository, ICountryRepository countryRepository, IBillingRepository billingRepository, IUsersCreditPaymentRepository usercreditpaymentRepository, IAdvertRepository advertRepository)
        {
        ...
        public JsonResult LoadData(DTParameters param)
        {
            try
            {
                ...
                var operatorId = _userRepository.GetAll().Where(top => top.VerificationStatus == true && (top.UserId == efmvcUser.UserId)).FirstOrDefault().OperatorId;
                ...
                if (searchValue == true)
                {
                    ...
                    //user = _userRepository.GetAll().Where(top => top.VerificationStatus == true && (top.RoleId == 3 && top.OperatorId == operatorId)).OrderByDescending(top => top.UserId);
                    advertUserId = _advertRepository.GetMany(top => top.OperatorId == operatorId).Select(top => top.UserId).ToList();
                    user = _userRepository.GetAll().Where(top => advertUserId.Contains(top.UserId)).OrderByDescending(top => top.UserId);
                    foreach (var item in user)
                    {
                        double creditlimit = 0;
                        //double outstandinginvoice = item.UsersCreditPayment.Count();
                        var mobileNumber = _contactsRepository.GetMany(a => a.UserId == item.UserId).FirstOrDefault().MobileNumber;
                        ...
                        _result.Add(new UserResult { Id = item.UserId, MobileNumber = mobileNumber, RoleId = item.RoleId, Role = role.ToString(), Email = item.Email, NoOfactivecampaign = item.CampaignProfiles.Where(top => campaignStatus.Contains(top.Status)).Count(), NoOfunapprovedadverts = item.Adverts.Where(top => top.Status == 4).Count(), Creditlimit = creditlimit, Outstandinginvoice = outstandinginvoice, CreatedDateSort = item.DateCreated, CreatedDate = item.DateCreated.ToString("dd/MM/yyyy"), FirstName = item.FirstName, LastName = item.LastName, status = item.Activated, fstatus = fstatus.ToString(), Name = item.FirstName + " " + item.LastName, TicketCount = item.Questions.Where(top => top.Status == 1 || top.Status == 2).Count() });
                    }
                    if (columnSearch[0] != null)
                    {
                ...
                {
                    //user = _userRepository.GetAll().Where(top => top.RoleId == 3 && top.OperatorId == operatorId).OrderByDescending(top => top.UserId);
                    advertUserId = _advertRepository.GetMany(top => top.OperatorId == operatorId).Select(top => top.UserId).ToList();
                    user = _userRepository.GetAll().Where(top => advertUserId.Contains(top.UserId)).OrderByDescending(top => top.UserId);
                    foreach (var item in user)
                    {
        ...
        public List<UserResult> FillUserResult()
        {
            ...
            var operatorId = _userRepository.GetAll().Where(top => top.VerificationStatus == true && (top.UserId == efmvcUser.UserId)).FirstOrDefault().OperatorId;
            //var result = _userRepository.GetAll().Where(top => top.RoleId == 3 && top.OperatorId == operatorId).OrderByDescending(top => top.UserId);
            List<int> advertUserId = _advertRepository.GetMany(top => top.OperatorId == operatorId).Select(top => top.UserId).ToList();
            var result = _userRepository.GetAll().Where(top => advertUserId.Contains(top.UserId)).OrderByDescending(top => top.UserId);
        ...
        public ActionResult FillUserDropdownAJAX(string UserName)
        {
            try
            {
                if (!string.IsNullOrEmpty(UserName))
                {
                    ...
                    var operatorId = _userRepository.GetAll().Where(top => top.VerificationStatus == true && (top.UserId == efmvcUser.UserId)).FirstOrDefault().OperatorId;
                    //var UserId = _userRepository.GetMany(top => top.RoleId == 3 && top.OperatorId == operatorId).Select(top=>top.UserId).ToList();
                    List<int> advertUserId = _advertRepository.GetMany(top => top.OperatorId == operatorId).Select(top => top.UserId).ToList();
                    //var userdetails = _userRepository.GetMany(top => (top.FirstName + " " + top.LastName).Contains(UserName) && top.RoleId == 3 && top.OperatorId == operatorId).Select(top => new
                    var userdetails = _userRepository.GetMany(top => (top.FirstName + " " + top.LastName).Contains(UserName) && top.RoleId == 3 && advertUserId.Contains(top.UserId)).Select(top => new
                    {
