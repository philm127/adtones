    ...
    public class LoginController : Controller
    {
        ...
        public ActionResult CheckLogin(LogOnFormModel form)
        {
            try
            {
                ...
                {
                    User user = _userRepository.Get(u => (u.Email == form.UserName) && u.RoleId == (int)UserRole.OperatorAdmin && u.Activated != 3);
                    if (user != null)
                    {
                    ...
                    if (usererror == 0)
                    {
                        if (ValidatePassword(user, form.Password))
                        {
                            ...
                            if (user.RoleId == (int)UserRole.OperatorAdmin)
                            {
