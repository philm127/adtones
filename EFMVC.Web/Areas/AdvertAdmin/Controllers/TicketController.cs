    ...
    public class TicketController : Controller
    {
        ...
        public JsonResult LoadData(DTParameters param)
        {
            try
            {
                ...
                if (searchValue == true)
                {
                    ...
                    help = _questionRepository.GetMany(s => (s.SubjectId == (int)QuestionSubjectStatus.Adreview || s.SubjectId == (int)QuestionSubjectStatus.AdvertError)).OrderByDescending(top => top.CreatedDate);
        ...
        private static List<TicketResult> ApplySorting(DTParameters param, List<TicketResult> result)
        {
            if (param.Order != null)
            {
                ...
                {
                    if (paramOrderDetails.Dir == DTOrderDir.ASC)
                        result = result.OrderBy(top => top.LastResponseDatetime).ToList();
                    else
                        result = result.OrderByDescending(top => top.LastResponseDatetime).ToList();
                }
            }
            return result;
        }
        ...
        public List<TicketResult> FillHelpResult()
        {
            ...
            // IOrderedEnumerable<Question> result = _questionRepository.GetMany(s=> (s.SubjectId == (int)QuestionSubjectStatus.Adreview)).OrderBy(top => top.Status).ThenByDescending(top => top.LastResponseDateTimeByUser);
            IOrderedEnumerable<Question> result = _questionRepository.GetMany(s => (s.SubjectId == (int)QuestionSubjectStatus.Adreview || s.SubjectId == (int)QuestionSubjectStatus.AdvertError)).OrderByDescending(top => top.CreatedDate);
        ...
        public void FillQuestionSubjectDropdown()
        {
            ...
            var Qsubjectdetails = _questionSubjectRepository.GetMany(s => (s.SubjectId == (int)QuestionSubjectStatus.Adreview || s.SubjectId == (int)QuestionSubjectStatus.AdvertError)).Select(top => new
            {
                Name = top.Name,
        ...
        public void FillUserDropdown()
        {
            ...
            var userdetails = _userRepository.GetMany(s => questionUserList.Contains(s.UserId)).Select(top => new
            {
        ...
        public List<TicketResult> gethelpResult(List<TicketResult> helpresult, TicketFilter _filterCritearea, int[] UserId, int[] ClientId, int?[] SubjectId, int[] StatusId, int[] PaymentMethodId, int[] CountryId, int[] OperatorId)
        {
            if (helpresult != null && _filterCritearea != null)
            {
                ...
                if ((_filterCritearea.LastResponseFromdate != null && _filterCritearea.LastResponseTodate != null))
                {
                    ...
                    helpresult = helpresult.Where(top => top.LastResponseDatetimeSort >= Fromdate.Value && top.LastResponseDatetimeSort <= Todate.Value).ToList();
                }
                if (SubjectId != null)
                {
                ...
                if (CountryId != null)
                {
                    ...
                    helpresult = helpresult.Where(top => contactUserIdData.Contains(top.userId)).ToList();
                }
                if (OperatorId != null)
                {
                    var userIdData = _userRepository.GetMany(top => OperatorId.Contains((int)top.OperatorId)).Select(top => top.UserId).ToList();
                    helpresult = helpresult.Where(top => userIdData.Contains(top.userId)).ToList();
                }
            }
            return helpresult;
        }
